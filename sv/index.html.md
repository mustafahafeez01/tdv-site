# Travel Document Vault | En sak mindre att oroa sig för

> Lagra pass, visum och ID säkert på din telefon. Få utgångspåminnelser månader i förväg. Offline-åtkomst, krypterad säkerhetskopia, inget konto.

Source: https://traveldocumentvault.com/sv/

---

09:41

## Resan du gör innan resan

Före den riktiga resan finns den här, den du gör i huvudet, där du går igenom sånt du redan har koll på.

Nu gör vi det ordentligt. Tio minuter, med start nu.

Ingen ser det här arbetet. Nu har det ett hem, den här resan och alla som kommer efter.

En sak mindre att oroa sig för.

![Ladda ned på App Store](https://traveldocumentvault.com/assets/images/app-store-badge-black.svg)

![Hämta på Google Play](https://traveldocumentvault.com/assets/images/google-play-badge.svg)

## Dina dokument stannar hos dig

Byggd av en förälder som behövde det. Dina dokument förblir på din enhet och läsbara endast för dig. Valfri krypterad säkerhetskopia går till ditt eget iCloud eller Google Drive, förseglat med en återställningskod endast du har.

### Byggd av en förälder

Jag byggde den här appen för att jag behövde den för min egen familj. Den har separata profiler för varje familjemedlem och automatiska utgångspåminnelser månader innan förnyelse är brådskande. Det finns ingen VC-finansiering som pressar mig att sälja dina data och ingen väg framåt driven av tillväxtmått - det är helt enkelt ett verktyg som löser ett problem på rätt sätt.

### Offline av design

Den säkeraste servern för passkanningar finns inte alls - vilket är varför appen behåller dokument på din enhet och fungerar helt offline som standard. Om du aktiverar säkerhetskopia går ditt krypterade valv till ditt eget iCloud eller Google Drive, förseglat med en återställningskod som endast du har.

### Kryptering på enheten

Dokument är krypterade på din enhet med AES-256-GCM, med nycklar som förvaras i din enhets Secure Enclave. Det är stark, modern kryptering, och du bör fortfarande skydda din telefon med ett starkt lösenord och App Lock.

### Din molnlagring. Din nyckel.

Valfri krypterad säkerhetskopia i ditt eget iCloud eller Google Drive. Du håller återställningskoden. Vi ser den aldrig. Synkronisera mellan dina enheter, men du förblir i kontroll.

### Smarta påminnelser

Pass varnar dig redan åtta månader i förväg, sedan kommer påminnelserna tätare - sex månader, tre, sex veckor, en månad - fram till utgångsdagen. Varje dokumenttyp har sitt eget schema, klart så snart du lägger till den.

### Familjemedveten logik

Vi följer reglerna åt dig. Olika länder, olika krav, barnpass förnyas snabbare. Appen förstår. Du behöver inte.

### Här är sanningen om vad jag kan se

Här är en ärlig redovisning av varje del av data i den här appen.

Dina passkanningar

På din enhet. Med valfri säkerhetskopia förseglad i ditt eget moln före uppladdning. Jag kan inte se dem på något sätt.

Dina dokumentdetaljer

Namn, nummer, utgångsdatum. Endast på din enhet.

Dina familjeprofiler

Aldrig överfört till mig. Det finns ingen Travel Document Vault-server för dem att nå.

Påminnelsemeddelanden

Schemalagd lokalt på din enhet. Jag känner inte till dina utgångsdatum.

Även om någon hackade mina servrar ... det finns inga servrar. Det finns ingenting att hitta.

[Läs grundarens berättelse: Varför jag byggde en app för att bära en mindre sak i mitt huvud →](https://traveldocumentvault.com/sv/blog/)

## Redo när du behöver dem

- **På flygplatsen:** Dra upp pass, ID och reseuppgifter för hela familjen på sekunder.
- **Fylla formulär:** Passnummer, ID-detaljer, försäkringsinformation. Alltid till hands.
- **Visumansökningar:** Exportera dokumentbilder som PDF för utskrift eller delning. Granska och justera bilder innan export.

## Planera resor, spåra kvoter, synkronisera till ditt eget moln

Byggd för familjer som reser överallt.

### Alla Klara, En Blick Bort

Pro

Beredskapstrafikljus för familjen. Se vem som är redo att resa och vems dokument som snart upphör.

### Din molnlagring. Din nyckel.

Pro

Krypterad säkerhetskopia i ditt iCloud eller Google Drive. Du håller återställningskoden. Vi ser den aldrig.

### Spåra Dagar Utomlands

Pro

Logga dagar som tillbringats i vilket land som helst per familjemedlem. Ställ in anpassade daggränsmål och vet exakt var du står.

### Resekvoter utan matematik

Pro

Spåra tullfria kvoter per familjemedlem per resa. Behöver aldrig gissa kring bagage- eller valutagränser igen.

## För resa. För livet. För allt som är viktigt.

Byggd för familjer som reser. Väsentlig för att hålla reda på alla vitala dokument, från försäkringsbrev till arbetsmärken.

### Smarta påminnelser

Passvarningar redan 8 månader i förväg

### Skanningsguider

Guider för att rama in pass och ID

### Kombinerade exporter

Kombinera dokument till en PDF för utskrift

### Talar ditt språk

Tillgänglig på över 40 språk. Appen känns naturlig var du än är.

🇬🇧 🇸🇦 🇮🇳 🇨🇳 🇹🇼 🇪🇸 🇧🇷 🇵🇹 🇫🇷 🇩🇪 🇯🇵 🇰🇷 🇮🇹 🇷🇺 🇳🇱 🇵🇱 🇹🇷 🇮🇩 🇻🇳 🇹🇭 🇺🇦 🇸🇪 🇳🇴 🇩🇰 🇫🇮 🇵🇰 🇮🇷 🇧🇩 🇷🇴 🇨🇿 🇭🇺 🇬🇷 🇸🇰 🇭🇷 🇸🇮 🇲🇾

### För hela familjen

Du är den som kommer ihåg allt. Den bördan behöver inte vila helt på dig. Organisera makar, barn och morföräldrar på ett säkert ställe.

### Sinnesro på autopilot

Vi bevakar datumen åt dig. Få diskreta påminnelser månader i förväg, så att förnyelsen är en checklista, inte en nödsituation.

### Skanna med säkerhet

Smidig dokumentinhämtning. Våra guider hjälper dig att få en klar, tydlig bild på sekunder, så att varje nummer och ansikte är läsbart.

### Ditt liv, sorterat

Pass, körkort, försäkring. Säkert lagrad, helt privat och spårad för utgång. Ett ställe för allt som är viktigt.

### Talar ditt språk

Tillgänglig på över 40 språk. Appen känns naturlig var du än är.

English, Español, Français, Deutsch, Italiano, Português (BR), Português (PT), Русский, 中文 (简体), 中文 (繁體), 日本語, 한국어, Nederlands, Polski, Türkçe, Indonesia, Tiếng Việt, ไทย, Українська, Svenska, Norsk, Dansk, Suomi

### Låt appen göra skrivandet

Omedelbar datumdetektering sparar dig från mödosam datainmatning. Snabbt, bekvämt och hållet 100% privat på din telefon.

### Ditt privata valv

Det som händer på din telefon stannar på din telefon. Det finns ingen molnserver involverad och ingen som tittar över din axel.

### Alltid där när du behöver det

Dåligt flygplats-Wi-Fi? Inga problem. Dina dokument är precis där på skärmen, vilket land du än är i. Fungerar utan problem i flygplansläge.

### Vi respekterar din integritet

Vi vill inte ha dina data. Vi säljer inte annonser. Vi vill bara hjälpa dig att resa bättre.

[Se alla funktioner](https://traveldocumentvault.com/sv/features/) [Osäker? Se varför familjer byter →](https://traveldocumentvault.com/sv/why-us/)

## Enkel, ärlig prissättning

Engångsköp. Inga prenumerationer. Inga dolda avgifter. Ingen datainsamling.

### Kostnadsfritt

Alltid gratis

- Pass, visum, ID och mer
- Skanna dokument, datum ifyllda för dig
- Utgångspåminnelser
- Dela enskilda dokument
- PIN + biometriskt lås (Face ID / Touch ID)
- 1 profil
- Upp till 5 dokument
- Nyligen raderat - 30-dagars ångerfönster

Alla data stannar på din enhet. Alltid.

Bäst värde

### Pro

Engångsköp

För hela din familj. Betala en gång. Alla v1.x-uppdateringar ingår för alltid.

[Om vår versionspolicy →](https://traveldocumentvault.com/pricing-policy/#version-policy)

- Varje familjemedlem, ett eget valv
- **Obegränsade** dokument
- Fullständig familjeutgångspanel
- Flersidiga dokument (upp till 10 sidor)
- Anpassad påminnelsetiming
- Kombinerad PDF-export för utskrift
- Exportera dokument i batch

### Kostnadsfritt

Allt du behöver för ensamresor

Gratis

- Pass, visum, ID och mer
- Skanna dokument, datum ifyllda för dig
- Utgångspåminnelser
- Dela enskilda dokument
- PIN + biometriskt lås (Face ID / Touch ID)
- 1 profil
- Upp till 5 dokument
- Nyligen raderat - 30-dagars ångerfönster

Alla data stannar på din enhet. Alltid.

För familjer

### Pro

Lås upp hela valvet för din familj

Engångsköp engångs För hela din familj. Betala en gång. Alla v1.x-uppdateringar ingår för alltid.

[Om vår versionspolicy →](https://traveldocumentvault.com/pricing-policy/#version-policy)

Allt i kostnadsfritt plus:

- Varje familjemedlem, ett eget valv
- **Obegränsade** dokument
- Fullständig familjeutgångspanel
- Flersidiga dokument (upp till 10 sidor)
- Anpassad påminnelsetiming
- Kombinerad PDF-export för utskrift
- Exportera dokument i batch

Återställ köp när som helst med det App Store- eller Google Play-konto du köpte med.

[Fullständig prissättningsinformation och vanliga frågor](https://traveldocumentvault.com/sv/pricing/)

## Vanliga frågor

Är det verkligen privat? Ja. Allt lagras 100% på din enhet. Vi har ingen åtkomst till dina dokument och det finns ingen molndatabas. Ta inte bara vårt ord för det - [kontrollera själv](https://traveldocumentvault.com/sv/privacy-verification/). Vad händer om jag tappar bort min telefon? Ditt valv lagras på din enhet. Om du byter telefon, återställ med din vanliga telefonbackup. Kan jag synkronisera mellan enheter? Ja. Med Pro, aktivera din molnlagring för att synkronisera ditt krypterade valv till ditt iCloud eller Google Drive. Du håller återställningskoden. Vi har aldrig åtkomst till dina data. Vad är reseplaneringen? Reseplaneringen grupperar familjens dokument efter resa och visar ett beredskapstrafikljus - grönt när alla är klara, gult om någons pass upphör snart. Planera resor tillsammans med självförtroende. [Se alla svar om sekretess och data](https://traveldocumentvault.com/sv/faq/)

**Viktigt:** Travel Document Vault är ett verktyg för personlig organisation för lagring av digitala kopior av dina dokument. **Digitala kopior som lagras i den här appen är INTE giltiga för resa.** Det verifierar inte dokumentets äkthet och ger inte juridisk eller reserådgivning. Bär alltid originaldokument och verifiera alla resekrav hos officiella myndigheter.

## Ställ in den i kväll

Ställ in den i kväll. Låt appen komma ihåg så att du inte behöver.

![Ladda ned på App Store](https://traveldocumentvault.com/assets/images/app-store-badge-black.svg)

![Hämta på Google Play](https://traveldocumentvault.com/assets/images/google-play-badge.svg)
