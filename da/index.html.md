# Travel Document Vault | En ting mindre at bekymre sig om

> Pas, visa, id-kort på enheden med udløbspåmindelser. Krypteret sikkerhedskopi til iCloud eller Google Drive. Offline, ingen konti, ingen servere.

Source: https://traveldocumentvault.com/da/

---

09:41

## Rejsen du tager før rejsen

Før den rigtige rejse er der denne, den du tager i hovedet, hvor du tjekker ting du allerede har tjekket.

Lad os gøre det ordentligt. Ti minutter, fra nu af.

Ingen ser dette arbejde. Nu har det et sted at høre til, denne rejse og alle de næste.

Én ting mindre at bekymre sig om.

![Hent på App Store](https://traveldocumentvault.com/assets/images/app-store-badge-black.svg)

![Hent det på Google Play](https://traveldocumentvault.com/assets/images/google-play-badge.svg)

## Dine Dokumenter Forbliver Hos Dig

Bygget af en forælder der havde brug for det. Dine dokumenter forbliver læsbare kun af dig. Valgfri krypteret sikkerhedskopi til din iCloud eller Google Drive er forseglede med en gendannelseskode som kun du har.

### Bygget af en forælder

Jeg byggede denne app fordi jeg havde brug for den til min egen familie. Den har separate profiler for hvert familiemedlem og automatiske udløbsmeddelelser måneder før fornyelse bliver presserende. Der er ingen VC-finansiering der presser mig til at sælge dine data, og ingen køreplan styret af væksthyg; det er simpelthen et værktøj der løser ét problem rigtigt.

### Offline efter design

Den sikreste server til paskopiering er slet ingen server, og det er grunden til, at appen holder dokumenter på din enhed og fungerer helt offline som standard. Hvis du slår sikkerhedskopi på, går dit krypterede lager til din egen iCloud eller Google Drive, forseglede med en gendannelseskode som kun du har.

### Kryptering på enheden

Dokumenter er krypteret på din enhed med AES-256-GCM ved hjælp af nøgler som holdes i din enheds Secure Enclave. Det er stærk, moderne kryptering, og du bør stadig beskytte din telefon med en stærk adgangskode og App Lock.

### Din Sky. Din Nøgle.

Valgfri krypteret sikkerhedskopi til din egen iCloud eller Google Drive, med engangsgenoprettelse på en ny telefon - du holder gendannelseskoden, så vi ser den aldrig. Gratis Vault Export er altid inkluderet.

### Smarte påmindelser

Pas advarer dig allerede otte måneder før, og derefter kommer påmindelserne tættere - seks måneder, tre, seks uger, en måned - frem til udløbsdagen. Hver dokumenttype har sin egen plan, klar så snart du tilføjer den.

### Familieopdelt logik

Vi sporer reglerne for dig. Forskellige lande, forskellige krav, børnepas fornys hurtigere. Appen forstår. Du behøver ikke.

### Her er sandheden om hvad jeg kan se

Her er en ærlig regnskab over hver eneste del af data i denne app.

Dine pas-scanninger

Lagret kun på din enhed. Jeg kan ikke se dem.

Dine dokument-detaljer

Navne, numre, udløbsdatoer. Kun på din enhed.

Dine familieprofile

Aldrig transmitteret til mig. Der er ingen Travel Document Vault-server for dem at nå.

Påmindelses-notifikationer

Planlagt lokalt på din enhed. Jeg kender ikke dine udløbsdatoer.

Selvom nogen hackede mine servere... der er ingen servere. Der er ikke noget at finde.

[Læs stifterens historie: Hvorfor jeg byggede en app til at bære en ting mindre i mit hoved →](https://traveldocumentvault.com/da/blog/)

## Planlæg ture, spor tillader, synkroniser til din egen sky

Bygget til familier der rejser overalt.

### Alle Klar, Med Ét Blik

Pro

Familiens beredskapstrafiklygt. Se hvem der er klar til at rejse, og hvis dokumenter snart udløber.

### Din Sky. Din Nøgle.

Pro

Krypteret sikkerhedskopi til din iCloud eller Google Drive. Du holder gendannelseskoden. Vi ser den aldrig.

### Spor Dage Uden for Landet

Pro

Log dage brugt i ethvert land pr. familiemedlem. Indstil brugerdefinerede dagsgrænsemål og ved præcist hvor du står.

### Toldfriomsætning Uden Matematik

Pro

Spor toldfriomsætninger per familiemedlem per rejse. Gæt aldrig igen på bagage- eller valutagrænser.

## Klar når du har brug for dem

- **I lufthavnen:** Få pas, id-kort og boardingdetaljer op for hele familien på få sekunder.
- **Udfyldelse af formularer:** Pasnumre, id-detaljer, forsikringsoplysninger. Altid ved dine fingerspidser.
- **Visa-ansøgninger:** Eksporter dokument billeder som PDF'er til udskrivning eller deling. Gennemgå og juster billeder før eksport.

## Til rejser. Til livet. Til alt der er vigtigt.

Bygget til familier der rejser. Essentiel for at holde styr på hvert vigtig dokument, fra forsikringspolicer til arbejdsmærker.

### Smarte påmindelser

Pasadvarsler allerede 8 måneder før

### Scanningsguides

Guider til at ramme pas og id-kort

### Kombinerede eksporter

Kombiner dokumenter til en PDF til udskrivning

### Taler dit sprog

Tilgængelig på over 40 sprog

🇬🇧 🇸🇦 🇮🇳 🇨🇳 🇹🇼 🇪🇸 🇧🇷 🇵🇹 🇫🇷 🇩🇪 🇯🇵 🇰🇷 🇮🇹 🇷🇺 🇳🇱 🇵🇱 🇹🇷 🇮🇩 🇻🇳 🇹🇭 🇺🇦 🇸🇪 🇳🇴 🇩🇰 🇫🇮 🇵🇰 🇮🇷 🇧🇩 🇷🇴 🇨🇿 🇭🇺 🇬🇷 🇸🇰 🇭🇷 🇸🇮 🇲🇾

### For hele familien

Du er den der husker alt. Den byrde behøver ikke hele tiden ligge på dine skuldre. Organiser ægtefæller, børn og bedsteforældre på ét sikkert sted.

### Sindsro, automatiseret

Vi passer datoerne så du ikke behøver. Få venlige dask måneder i forvejen, så fornyelse er en checklistepunkt, ikke en nødssituation.

### Scan med tillid

Strømlinet optagelse. Vores guider hjælper dig med at få et klart, skarpt billede på få sekunder, så hvert nummer og ansigt er læseligt.

### Dit liv, organiseret

Pas, kørekort, forsikring. Sikkert opbevaret, strengt privat og sporet for udløb. Ét sted til alt, der betyder noget.

### Taler dit sprog

Tilgængelig på over 40 sprog. Appen føles naturlig, uanset hvor du er.

English, Español, Français, Deutsch, Italiano, Português (BR), Português (PT), Русский, 中文 (简体), 中文 (繁體), 日本語, 한국어, Nederlands, Polski, Türkçe, Indonesia, Tiếng Việt, ไทย, Українська, Svenska, Norsk, Dansk, Suomi

### Lad appen skrive for dig

Øjeblikkelig datoregistrering sparer dig for besværlig dataindtastning. Hurtig, praktisk og 100% privat på din telefon.

### Dit private pengeskab

Det der sker på din telefon, bliver på din telefon. Ingen cloud-servere. Ingen nysgerrige øjne. Kun dig.

### Altid der, når du har brug for det

Ustabilt lufthavn-Wi-Fi? Intet problem. Dine dokumenter er lige der på skærmen, uanset hvilket land du er i. Fungerer problemfrit i flytilstand.

### Respekterer dit privatliv

Vi vil ikke have dine data. Vi sælger ikke reklamer. Vi vil bare hjælpe dig med at rejse bedre.

[Se alle funktioner](https://traveldocumentvault.com/da/features/) [Ikke sikker? Se hvorfor familier skifter →](https://traveldocumentvault.com/da/why-us/)

## Enkelt, ærlig prissætning

Engangskøb. Ingen abonnementer. Ingen skjulte gebyrer. Ingen dataindsamling.

### Gratis

Altid gratis

- Pas, visa, id-kort og mere
- Scan dokumenter, datoer udfyldes for dig
- Udløbspåmindelser
- Del individuelle dokumenter
- PIN + biometrisk lås (Face ID / Touch ID)
- 1 profil
- Op til 5 dokumenter

Alle data forbliver på din enhed. Altid.

Bedste værdi

### Pro

$9.99

Til hele din familie. Betal én gang. Alle v1.x-opdateringer inkluderet for evigt.

[Om vores versionspolitik →](https://traveldocumentvault.com/pricing-policy/#version-policy)

- Rejseplaner med familieberedes-trafiklygt
- Din Cloud-sikkerhedskopi (iCloud eller Google Drive, end-to-end krypteret)
- Tracker for dage i udlandet til skattedag og visagränser
- Rejsetilladelsesgränser - toldfri, bagage, valuta
- Hvert familiemedlem, ét pengeskab
- **Ubegrænsede** dokumenter
- Komplet familie-udløbsoversigt
- Flersidede dokumenter (op til 10 sider)
- Tilpasset påmindelsestiming
- Kombineret PDF-eksport til udskrivning
- Batch-eksport af dokumenter

### Gratis

Alt hvad du behøver til solorejser

Gratis

- Pas, visa, id-kort og mere
- Scan dokumenter, datoer udfyldes for dig
- Udløbspåmindelser
- Del individuelle dokumenter
- PIN + biometrisk lås (Face ID / Touch ID)
- 1 profil
- Op til 5 dokumenter
- Nylig slettet - 30-dagers fortryd vindue

Alle data forbliver på din enhed. Altid.

Til familier

### Pro

Lås det fulde pengeskab op til din familie

$9.99 engangsbetaling Til hele din familie. Betal én gang. Alle v1.x-opdateringer inkluderet for evigt.

[Om vores versionspolitik →](https://traveldocumentvault.com/pricing-policy/#version-policy)

Alt i Gratis, plus:

- Rejseplaner med familieberedes-trafiklygt
- Din Cloud-sikkerhedskopi (iCloud eller Google Drive, end-to-end krypteret)
- Tracker for dage i udlandet til skattedag og visagränser
- Rejsetilladelsesgränser - toldfri, bagage, valuta
- Hvert familiemedlem, ét pengeskab
- **Ubegrænsede** dokumenter
- Komplet familie-udløbsoversigt
- Flersidede dokumenter (op til 10 sider)
- Tilpasset påmindelsestidspunkt
- Samlet PDF-eksport til udskrivning
- Masseeksport af dokumenter

Gendan køb når som helst med den App Store eller Google Play-konto, du købte med.

[Fuld prisdetaljer og ofte stillede spørgsmål](https://traveldocumentvault.com/da/pricing/)

## Ofte stillede spørgsmål

Er det virkelig privat? Ja. Alt lagres 100% på din enhed. Vi har ingen adgang til dine dokumenter, og der er ingen cloud-database. Tag ikke bare vores ord for det - [tjek det selv](https://traveldocumentvault.com/da/privacy-verification/). Hvad hvis jeg mister min telefon? Dit vault er gemt på din enhed. Almindelige telefonsikkerhedskopier (iCloud/Google Backup) genopretter ikke app-data, fordi krypteringsnøglen aldrig forlader din enhed. I stedet eksporterer du en krypteret .tdvault-fil (gratis) eller bruger Pro-Cloud Backup til dit eget iCloud/Google Drive. Kan jeg synkronisere mellem enheder? Ja. Med Pro skal du aktivere Din Cloud-sikkerhedskopi for at synkronisere din krypterede boks til din iCloud eller Google Drive. Du holder gendannelseskoden. Vi får aldrig adgang til dine data. Hvad er rejseplanlæggeren? Rejseplanlæggeren grupperer familien dokumenter efter rejse og viser en beredelsestrafiklygt - grøn når alle er klar, gul hvis nogen's pas snart udløber. Planlæg rejser sammen med selvtillid. [Se alle svar om privatliv og data](https://traveldocumentvault.com/da/faq/)

**Vigtigt:** Travel Document Vault er et personligt organisationsværktøj til opbevaring af digitale kopier af dine dokumenter. **Digitale kopier gemt i denne app er IKKE gyldige til rejse.** Den verificerer ikke dokumenters ægthed og yder ikke juridisk eller rejserelateret rådgivning. Medbring altid originale dokumenter og bekræft alle rejsekrav hos officielle myndigheder.

## Sæt det op i aften

Sæt det op i aften. Lad appen huske, så du ikke behøver.

![Hent på App Store](https://traveldocumentvault.com/assets/images/app-store-badge-black.svg)

![Hent det på Google Play](https://traveldocumentvault.com/assets/images/google-play-badge.svg)
